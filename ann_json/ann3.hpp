...
#ifndef ANN3_HPP_INCLUDED
#define ANN3_HPP_INCLUDED
...
namespace ann3 {
  ...
  namespace activation {
    ...
    struct linear : state<1>
    {
      ...
      template <typename T>
      static T apply(T u, const T* __restrict state)
      {
        return u * state[0];
      }
    };
  ...
  public:
    ...
    value_type& operator[](size_t i) { assert(i < S); return *(begin_ + i); }
    value_type operator[](size_t i) const { assert(i < S); return *(begin_ + i); }
  ...
  public:
    ...
    explicit NeuronView(T* state) noexcept : state_(state) {}
  ...
  public:
    ...
    static constexpr size_t size = type::size;    // number of neurons

    explicit LayerView(T* state) noexcept : state_(state) {}
    
    state_view<T, state_size> state() noexcept { return state_view<value_type, state_size>(state); }
    state_view<T, state_size> state() const noexcept { return state_view<value_type, state_size>(state); }

    neuron_view operator[](size_t i) { assert(i < size); return neuron_view(state_ + i * type::neuron_t::state_size); }
    neuron_view operator[](size_t i) const { assert(i < size); return neuron_view(state_ + i * type::neuron_t::state_size); }
  ...
  public:
    ...
    auto get_layer() const
    {
      static_assert(I < std::tuple_size<layer_t>::value, "NetworkImpl::get_layer index out of range");
      using layerI = std::tuple_element_t<I, layer_t>;
      return LayerView<const T, layerI>(state_.data() + detail::accum_state_ofs<I, layer_t>::value);
    }
    ...
    // feed forward, array input
    output_t operator()(const input_t & in)
    {
      return do_feed_forward<0>(in);
    }

    // feed forward, iterator input
    template <typename FwIt>
    auto operator()(FwIt first, FwIt last)
      -> std::enable_if_t< std::is_convertible_v<typename std::iterator_traits<FwIt>::value_type, value_type>, output_t>
    {
      assert(std::distance(first, last) == input_size);
      input_t in; std::copy(first, last, in.begin());
      return do_feed_forward<0>(in);
    }

    // feed forward, separated inputs
    template <typename ... INPUT>
    auto operator()(INPUT... in)
      -> std::enable_if_t<std::conjunction_v<std::is_convertible<value_type, INPUT>...>, output_t>
    {
      static_assert(sizeof...(INPUT) == input_size, "NetworkImpl::operator(): illegal input pack");
      return do_feed_forward<0>({ value_type(in)... });
    }

    // comparison operators, lexicographically
    friend bool operator==(const network_t& a, const network_t& b) { return a.state_ == b.state_; }
    friend bool operator!=(const network_t& a, const network_t& b) { return a.state_ != b.state_; }
    friend bool operator<(const network_t& a, const network_t& b) { return a.state_ < b.state_; }
    friend bool operator<=(const network_t& a, const network_t& b) { return a.state_ <= b.state_; }
    friend bool operator>(const network_t& a, const network_t& b) { return a.state_ > b.state_; }
  ...
  namespace detail {
    ...
    inline auto do_visit_layer(network_t& network, Visitor&& visitor)
      -> std::enable_if_t<I == network_t::output_layer + 1>
    {
    }
...
#if defined(_MSC_VER) && _MSC_VER <= 1900 && !defined(__c2__)
      visitor.operator()(layer, I);
#else
      visitor.template operator()(layer, I);
#endif
      do_visit_layer<I + 1>(network, std::forward<Visitor>(visitor));
    }
  ...
  // The visitor shall be a callable object with the following 
  // signature:
  //
  // template <typename LayerView>
  // void operator()(LayerView layer_view, size_t layer)
  //
  ...
  // void operator()(NeuronView neuron_view, size_t layer, size_t node)
  //
  template <typename network_t, typename Visitor>
